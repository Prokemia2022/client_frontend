...
function FilterExperts({isfilterexpertsModalvisible,setisfilterexpertsModalvisible}){
    const { isOpen, onOpen, onClose } = useDisclosure();

    const HandleModalOpen=()=>{
      if(isfilterexpertsModalvisible !== true){
      }else{
        onOpen();
        setisfilterexpertsModalvisible(false)
      }
    }

    useEffect(()=>{
      HandleModalOpen();
    },[isfilterexpertsModalvisible])
