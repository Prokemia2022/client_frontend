import { Drawer, DrawerBody, DrawerFooter, DrawerHeader, DrawerOverlay, DrawerContent, DrawerCloseButton, Button, Image, Text, Flex, Box, Icon, HStack, useDisclosure, Divider, Select, FormControl, FormLabel, FormErrorMessage, Textarea, Input, useToast,
...
const Make_A_Quote=({prod_data,view_drawer_disclosure})=>{
    ...
    const Handle_Sumbit=async()=>{
        set_saving(true)
        if (!user){
            toast({title:'Warning!',description:'You need to be signed in to request a quote',status:'warning',position:'top-left-accent',variant:'left-accent',isClosable:true})
            set_saving(false)
            return ;
        }
        if (!payload?.address || !industry || !technology || !description){
