...
import { Text,  Box,  Breadcrumb, BreadcrumbItem, BreadcrumbLink, FormControl, FormLabel, FormErrorMessage, VStack, Input, Textarea, HStack, Select, Divider, Wrap, Button, Tooltip, useToast, Avatar, Popover, PopoverTrigger, Portal, PopoverContent, PopoverArrow, PopoverCloseButton, PopoverBody, Alert, AlertIcon, AlertTitle, AlertDescription, Switch } from '@chakra-ui/react';
//components
import Edit_documents from "./edit_documents.js";
//apis
...
//payload 
    ...
    //useEffects
	useEffect(()=>{
        if (product_page_data?._id ){
            get_Industries_Data()
            get_Technologies_Data()
            get_Product_Data()
        }
    },[product_page_data?._id])
        ...
        <Box py='2'>
            <VStack justifyContent={'space-between'} align='flex-start'>
                ...
                <Breadcrumb spacing='5px' alignItems={'center'} fontSize={'sm'} fontWeight={'semibold'}>
                    <BreadcrumbItem onClick={(()=>{set_page('Inventory')})}>
                ...
                <Box>
                    ...
                    <Box bg='#fff' borderRadius={8} mt='4' p='4'>
                        ...
                        <FormControl display='flex' alignItems='center' mt='2' gap='2'>
                            <MdInfoOutline style={{cursor:'pointer'}} onClick={(()=>{set_short_on_expiry_status_info(!short_on_expiry_status_info)})}/>
                    ...
                    <Box mt='2' align='end' gap='2'>
                        ...
                        <Tooltip hasArrow label='Save product details'  placement='auto'>
                            {is_submitting? <Button isLoading loadingText="saving..." colorScheme="teal"/> : <Button ml={'2'} bg='#009393' color='#fff' onClick={Handle_Edit_Product}>Save product</Button> }
                        </Tooltip>
                    </Box>
                </Box>
            }
