...
import { Box, Button, Divider, FormControl, FormLabel, HStack, IconButton, Input, Text, Tooltip, useToast } from '@chakra-ui/react';
...
//api
import { Edit_Product } from '../../../../pages/api/product/route.api.js';
//utils
    ...
    <Box>
                        ...
                        {is_data_sheet_existing === ''?
                            <HStack mt='2'>
                                ...
                                <Tooltip label={'Add document'} placement='auto'>
                                    <IconButton aria-label='Change File' icon={<MdPostAdd />} onClick={(()=>{set_is_change_data_sheet(true)})}/>
                                </Tooltip>
                            </HStack>
                        :
                    ...
                    <>
                        {is_safety_data_sheet_existing === ''?
                            <HStack mt='2'>
                                ...
                                <Tooltip label={'Add document'} placement='auto'>
                                    <IconButton aria-label='Change File' icon={<MdPostAdd />} onClick={(()=>{set_is_change_safety_data_sheet(true)})}/>
                                </Tooltip>
                            </HStack>
                        :
                    ...
                    <>
                        {is_formulation_document_existing === ''?
                            <HStack mt='2'>
                                ...
                                <Tooltip label={'Add document'} placement='auto'>
                                    <IconButton aria-label='Change File' icon={<MdPostAdd />} onClick={(()=>{set_is_change_formulation_document(true)})}/>
                                </Tooltip>
                            </HStack>
                        :
        ...
        <Box mt='2' align='end' gap='2'>
            <Tooltip hasArrow label='Go back to editing the product details'  placement='auto' >
                <Button leftIcon={<FaArrowLeft />} onClick={(()=>{set_iseditfiles(false)})}>Edit product</Button>
            </Tooltip>
...
const Existing_files_Card_Item=({name,set_is_change_data_sheet,set_is_change_safety_data_sheet,set_is_change_formulation_document})=>{
	...
	return(
		<HStack justify='space-between' mt='2' border='1px' borderColor='gray.200' p='2' borderRadius='md'>
            <HStack>
                <MdInsertDriveFile style={{color:"orange"}}/>
                <Text w='100%' >{name}</Text>
            </HStack>
            <HStack>
                <Tooltip label={`Change ${name}`} placement='auto'>
                    <IconButton aria-label='Change File' icon={<LuFileEdit />} onClick={handle_file_change}/>
                </Tooltip>
            </HStack>
        </HStack>
	)
...
const Selected_Card_Item=({name,file_type,set_data_sheet,set_formulation_document,set_safety_data_sheet,is_submitting})=>{
	...
	return(
		<HStack justify='space-between'>
            <HStack>
                <MdOutlineDescription style={{color:"orange"}}/>
                <Text w='100%' >{name}</Text>
            </HStack>
