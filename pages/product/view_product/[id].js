...
function Product(){
	...
	return(
		<Flex direction='column'>
			<Header />
			{edit?
				<Edit_Info product_data={product_data} set_edit={set_edit}/>
				:
				<Flex direction='column'>
...
const Edit_Info=({product_data,set_edit})=>{
	...
	const handle_edit_product=async()=>{
		...
		await Edit_Product(payload).then(()=>{
			alert("successfully edited this product")
			set_edit(false)
			router.back()
		})
	}
	...
	return(
		<Flex className={styles.productbody} direction='column' p='3' gap='3'>
                ...
                <Button bg='#eee' borderRadius='0' color='red' onClick={(()=>{set_edit(false)})}>Cancel</Button>
		</Flex>
	)
}