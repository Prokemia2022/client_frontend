...
//usestates:
	...
	//payload 
		...
		const handle_add_new_product=async()=>{
			...
			*/
			if (is_acc_verified == 'false'){
					toast({
						title: 'Could not add a new listing.',
						position: 'top-left',
						variant:"subtle",
			...
			if (is_acc_suspended == 'true'){
					toast({
						title: 'Could not add a new listing.',
						position: 'top-left',
						variant:"subtle",
						description: `Your account has been suspended, email support at help@prokemia.com.`,
						status: 'info',
						isClosable: true,
					});
				return ;
		...
		useEffect(()=>{
			if (!token){
				toast({
					...
					position: 'top-left',
					variant:"subtle",
					description: `You need to have an account, to list a product. We are redirecting you.`,
					status: 'info',
					isClosable: true,
				});
				router.back()
