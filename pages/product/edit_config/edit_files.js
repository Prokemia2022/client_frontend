...
export default function EditFile({product_data,payload,set_iseditfiles}){
	...
	const [safety_data_sheet_url,set_safety_data_sheet_url]=useState(product_data?.safety_data_sheet_url);
	...
	const [formulation_document_url,set_formulation_document_url]=useState(product_data?.formulation_document_url);
	...
	const edit_payload = {
		...
		data_sheet: data_sheet_url,
		safety_data_sheet : safety_data_sheet_url,
		formulation_document : formulation_document_url,
	}
	...
	const handle_File_Upload=async()=>{
		...
		if (data_sheet !== ''){
			handle_data_sheet_file_upload().then((res)=>{
				console.log(res)
		...
		if (safety_data_sheet !== ''){
			handle_safety_sheet_file_upload().then((res)=>{
				console.log(res)
		...
		if (formulation_document !== ''){
			handle_formulation_document_file_upload().then((res)=>{
				console.log(res)
	...
	const Edit_Product_Function=async()=>{
		...
		if (data_sheet !== '' && edit_payload.data_sheet_url !== product_data?.data_sheet){
			set_data_sheet_url(cookies.get("data_sheet_url"))
			set_is_retry(true)
		}else if (safety_data_sheet !== '' && edit_payload.safety_data_sheet_url !== product_data?.safety_data_sheet_url){
			set_safety_data_sheet_url(cookies.get("safety_data_sheet_url"))
			set_is_retry(true)
		}else if (formulation_document !== '' && edit_payload.formulation_document_url !== product_data?.formulation_document_url){
			set_formulation_document_url(cookies.get("formulation_document_url"))
			set_is_retry(true)
		}
		if ((data_sheet_url !== product_data?.data_sheet) || (safety_data_sheet_url !== product_data?.safety_data_sheet) || (formulation_document_url !== product_data?.formulation_document)){
			set_is_submitting(true)
				...
				{is_upload_file?
					<Flex direction='column' boxShadow='lg' p='1' gap='2' m='2'>
						...
						<Flex m='2' gap='2' color='#fff'>
							{is_retry?
								<Button flex='1' bg='#000' onClick={Edit_Product_Function} disabled={is_submitting?true:false}>Complete</Button>
								:
		...
		<Flex direction='column'  p='1' gap='2' m='2'>
			...
			:
				<Flex justify='space-between' boxShadow='lg' p='2' align='center' borderRadius='5'> 
					...
					<Link isExternal href={product_data?.data_sheet} border='1px solid grey' p='2' borderRadius='5'>View</Link>
				</Flex>
			}
			...
			:
				<Flex justify='space-between' boxShadow='lg' p='2' align='center' borderRadius='5'> 
					...
					<Link isExternal href={product_data?.safety_data_sheet} border='1px solid grey' p='2' borderRadius='5'>View</Link>
				</Flex>
			}
			...
			:
				<Flex justify='space-between' boxShadow='lg' p='2' align='center' borderRadius='5'> 
					<InsertDriveFileIcon/>
					<Text>Formulation document</Text>
					<Link isExternal href={product_data?.formulation_document} border='1px solid grey' p='2' borderRadius='5'>View</Link>
				</Flex>
			}
			<Flex gap='2'>
