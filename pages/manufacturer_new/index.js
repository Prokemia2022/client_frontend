...
import Manufacturers from './distributors';
...
export default function index() {
    ...
    const Handle_Email_Verification=async()=>{
      ...
      const email_payload={
        ...
        code: code,
        link: `https://prokemia.com/verify/${'manufacturer'}/${client_data._id}`
      }
      await Email_Verification(email_payload).then(()=>{
        router.push(`/verify/${'manufacturer'}/${client_data._id}`)
      }).catch((err)=>{
...
const Body=(props)=>{
  ...
  }else if (current_page == 'distributors'){
    return (
