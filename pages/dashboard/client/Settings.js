...
import Manage from "./manage/client";
import { useUserContext } from "../../../components/Providers/userContext";
import DeleteAccount from "../../../components/ui/Dashboard/delete_account.ui";
import {storage} from '../../../lib/firebase.js';
import {ref,uploadBytes,getDownloadURL} from 'firebase/storage';
import { v4 } from "uuid";
import { useRouter } from "next/router.js";
import { Update_Client } from "../../api/clients/client/route.api.js";
        ...
        <Box>
            ...
            <HStack bg='#fff' p='4' alignItems='center' borderRadius={'md'} boxShadow={'sm'} mt='2'>
                <Tooltip hasArrow placement='auto' label={user?.address === '' || user?.mobile_of_company === ''? 'Almost there! lets complete your profile' : 'click icon to edit your account'} >
                    <Avatar border={user?.address === '' || user?.mobile_of_company === ''? '2px dashed orange':''} p={user?.address === '' || user?.mobile_of_company === ''? .5 : ''} size={{ base:'lg', md:'2xl' }} bg={user?.address === '' || user?.mobile_of_company === ''? 'orange.200':''} src={user?.profile_photo_url} onClick={(()=>set_profile_edit(!profile_edit))}>
                        {profile_edit?
                ...
                {profile_edit? 
                    <Flex direction='column' gap='2'>
                        ...
                        {is_saving? <Button isLoading loadingText="Saving..."/> : <Button bg='#009393' color='#fff' disabled={profile_photo == ''? true: false} onClick={HandleImageUpload}>save photo</Button>}
                    </Flex>
                :
                    <Box flex='1'>
